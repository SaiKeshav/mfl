...
class GENREHubInterface(BARTHubInterface):
    def sample(
        ...
        **kwargs,
        ...
        # actual dictionary contains 255997 elements
        # 4 special characters are added - <s>, </s>, <pad>, <unk>, embedding size: 256001
        # but there seems to be 2 more extra charcters in self.task.source_dictionary, dict size: 256003
        # replacing them with unk
        tokenized_sentences = [torch.tensor([t if t < 256001 else 3 for t in self.encode(sentence)]) for sentence in sentences]
