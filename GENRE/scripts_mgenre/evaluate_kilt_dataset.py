...
#                 [create_input(doc, max_len_a, ) for doc in docs],
                [html.unescape(' '.join(doc['input'].split()[:max_len_a])) for doc in docs],
                beam=beams,
                lenpen=lenpen,
                max_len_b=max_len_b,
...
#             gold.append(doc["output"][0]["answer"])
            gold.append([doc["gold"]])

            final_answer = [None]
            for dp in doc["predictions"]:
                if dp["answer"] != [None]:
                    final_answer = doc["predictions"][0]["answer"]
                    break
...
#     ), "`input_path` and `output_path` have either to be both files or folders"
    ...
    model = (
        mGENRE.from_pretrained(
            ...
            # sentencepiece_model='sentence.bpe.model', # base intizn    
            sentencepiece_model='spm_256000.model'
#             sentencepiece_model=os.path.join(args.model_path, "spm_256000.model"), # mgenre initzn
        )
        .eval()
        .to(args.device)
    )
